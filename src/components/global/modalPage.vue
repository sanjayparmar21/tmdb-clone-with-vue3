<template>
        <section v-if="this.$store.state.movieId" class="bg-black bg-opacity-50 w-full fixed inset-0" @click="close">
            <section :style="{ backgroundImage: `url(${this.prefix + details.backdrop_path})` }"
                class="bg-cover z-10 text-white bg-slate-100 max-w-5xl fixed right-0 top-14 bottom-0 rounded-tl-xl rounded-bl-xl">
            <!-- <section  v-if="this.$store.state.movieId" :style="{ backgroundImage: `url(${this.prefix + details.backdrop_path})` }" class="bg-cover z-10 text-white bg-slate-100 max-w-5xl fixed right-0 top-14 bottom-0 rounded-tl-xl rounded-bl-xl"> -->
                <section class="bg-black bg-opacity-40 min-h-full rounded-tl-xl rounded-bl-xl" >
                    <article class="justify-center block sm:flex items-start max-w-7xl m-auto rounded-tl-xl p-6">
                        ...
                        <article class="ml-10">
                            ...
                            <div :class="this.loading ? 'flex flex-row items-center gap-2 sm:gap-4 my-4 skeleton-for-vote' : 'flex flex-row items-center gap-2 sm:gap-4 my-4'">
